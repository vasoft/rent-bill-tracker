...
import { type SsvSpaceRow } from '@/hooks/use-ssv-distribution';

interface SsvTableProps {
  ssvData: SsvSpaceRow[];
  isInitialized: boolean;
  isConfirmed: boolean;
  onDataChange?: (updatedData: SsvSpaceRow[]) => void;
}

const fmt = (n: number) => n.toLocaleString('ro-RO', { minimumFractionDigits: 2 });

export default function SsvTable({ ssvData, isInitialized, isConfirmed, onDataChange }: SsvTableProps) {
  const [viewRow, setViewRow] = useState<SsvSpaceRow | null>(null);
  const [editRow, setEditRow] = useState<SsvSpaceRow | null>(null);
  ...
  const totals = {
    consumReferinta: ssvData.reduce((s, r) => s + r.consumReferinta, 0),
    cantitateAlocata: ssvData.reduce((s, r) => s + r.cantitateAlocata, 0),
    valoareNeta: ssvData.reduce((s, r) => s + r.valoareNeta, 0),
    valoareTva: ssvData.reduce((s, r) => s + r.valoareTva, 0),
    total: ssvData.reduce((s, r) => s + r.total, 0),
  };

  const handleEdit = (row: SsvSpaceRow) => {
    setEditRow(row);
    ...
    <>
      ...
      <Dialog open={!!viewRow} onOpenChange={() => setViewRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Detalii Supraveghere Video</DialogTitle>
          ...
          {viewRow && (
            <div className="space-y-3 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Consum referință (Suprafață)</p>
                  <p className="font-mono font-semibold">{fmt(viewRow.consumReferinta)} mp</p>
                </div>
                <div>
      ...
      <Dialog open={!!editRow} onOpenChange={() => setEditRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Modificare Supraveghere Video</DialogTitle>
