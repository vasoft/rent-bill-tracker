...
import { type ScSpaceRow } from '@/hooks/use-sc-distribution';

interface ScTableProps {
  scData: ScSpaceRow[];
  isInitialized: boolean;
  isConfirmed: boolean;
  onDataChange?: (updatedData: ScSpaceRow[]) => void;
}

const fmt = (n: number) => n.toLocaleString('ro-RO', { minimumFractionDigits: 2 });

export default function ScTable({ scData, isInitialized, isConfirmed, onDataChange }: ScTableProps) {
  const [viewRow, setViewRow] = useState<ScSpaceRow | null>(null);
  const [editRow, setEditRow] = useState<ScSpaceRow | null>(null);
  ...
  const totals = {
    consumReferinta: scData.reduce((s, r) => s + r.consumReferinta, 0),
    cantitateAlocata: scData.reduce((s, r) => s + r.cantitateAlocata, 0),
    valoareNeta: scData.reduce((s, r) => s + r.valoareNeta, 0),
    valoareTva: scData.reduce((s, r) => s + r.valoareTva, 0),
    total: scData.reduce((s, r) => s + r.total, 0),
  };

  const handleEdit = (row: ScSpaceRow) => {
    setEditRow(row);
    ...
    <>
          ...
          {scData.length === 0 ? (
            ...
            scData.map(row => (
              <TableRow key={row.spaceId} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <Badge variant="outline" className="gap-1.5 bg-chart-sc/10 text-chart-sc border-chart-sc/30">
                    <Calculator className="w-4 h-4" />
                    SC
                  </Badge>
                </TableCell>
                <TableCell className="text-right font-mono">
                  {fmt(row.consumReferinta)} pers
                </TableCell>
        ...
        {scData.length > 0 && (
          <TableFooter>
            <TableRow className="font-semibold">
              ...
              <TableCell className="text-right font-mono">{fmt(totals.consumReferinta)} pers</TableCell>
      ...
      <Dialog open={!!viewRow} onOpenChange={() => setViewRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Detalii Serviciu Curățenie</DialogTitle>
          ...
          {viewRow && (
            <div className="space-y-3 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Consum referință (Persoane)</p>
                  <p className="font-mono font-semibold">{fmt(viewRow.consumReferinta)} pers</p>
                </div>
                <div>
      ...
      <Dialog open={!!editRow} onOpenChange={() => setEditRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Modificare Serviciu Curățenie</DialogTitle>
          ...
          {editRow && (
            <div className="space-y-4 py-4">
              <div className="p-3 bg-muted rounded-lg">
                <p className="text-sm text-muted-foreground">Consum referință (Persoane)</p>
                <p className="font-mono font-semibold">{fmt(editRow.consumReferinta)} pers</p>
              </div>
