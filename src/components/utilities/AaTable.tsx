...
import { type AaSpaceRow } from '@/hooks/use-aa-distribution';

interface AaTableProps {
  aaData: AaSpaceRow[];
  isInitialized: boolean;
  isConfirmed: boolean;
  onDataChange?: (updatedData: AaSpaceRow[]) => void;
}

const fmt = (n: number) => n.toLocaleString('ro-RO', { minimumFractionDigits: 2 });

export default function AaTable({ aaData, isInitialized, isConfirmed, onDataChange }: AaTableProps) {
  const [viewRow, setViewRow] = useState<AaSpaceRow | null>(null);
  const [editRow, setEditRow] = useState<AaSpaceRow | null>(null);
  ...
  const totals = {
    consumReferinta: aaData.reduce((s, r) => s + r.consumReferinta, 0),
    cantitateAlocata: aaData.reduce((s, r) => s + r.cantitateAlocata, 0),
    valoareNeta: aaData.reduce((s, r) => s + r.valoareNeta, 0),
    valoareTva: aaData.reduce((s, r) => s + r.valoareTva, 0),
    total: aaData.reduce((s, r) => s + r.total, 0),
  };

  const handleEdit = (row: AaSpaceRow) => {
    setEditRow(row);
    ...
    <>
          ...
          {aaData.length === 0 ? (
            ...
            aaData.map(row => (
              <TableRow key={row.spaceId} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <Badge variant="outline" className="gap-1.5 bg-chart-aa/10 text-chart-aa border-chart-aa/30">
      ...
      <Dialog open={!!viewRow} onOpenChange={() => setViewRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Detalii Analize Ape Uzate</DialogTitle>
          ...
          {viewRow && (
            <div className="space-y-3 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Consum referință (A&C)</p>
      ...
      <Dialog open={!!editRow} onOpenChange={() => setEditRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Modificare Analize Ape Uzate</DialogTitle>
