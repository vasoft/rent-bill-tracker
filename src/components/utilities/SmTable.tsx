...
import { type SmSpaceRow } from '@/hooks/use-sm-distribution';

interface SmTableProps {
  smData: SmSpaceRow[];
  isInitialized: boolean;
  isConfirmed: boolean;
  onDataChange?: (updatedData: SmSpaceRow[]) => void;
}

const fmt = (n: number) => n.toLocaleString('ro-RO', { minimumFractionDigits: 2 });

export default function SmTable({ smData, isInitialized, isConfirmed, onDataChange }: SmTableProps) {
  const [viewRow, setViewRow] = useState<SmSpaceRow | null>(null);
  const [editRow, setEditRow] = useState<SmSpaceRow | null>(null);
  ...
  const totals = {
    consumReferinta: smData.reduce((s, r) => s + r.consumReferinta, 0),
    cantitateAlocata: smData.reduce((s, r) => s + r.cantitateAlocata, 0),
    valoareNeta: smData.reduce((s, r) => s + r.valoareNeta, 0),
    valoareTva: smData.reduce((s, r) => s + r.valoareTva, 0),
    total: smData.reduce((s, r) => s + r.total, 0),
  };

  const handleEdit = (row: SmSpaceRow) => {
    setEditRow(row);
    ...
    <>
          ...
          {smData.length === 0 ? (
            ...
            smData.map(row => (
              <TableRow key={row.spaceId} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <Badge variant="outline" className="gap-1.5 bg-chart-sm/10 text-chart-sm border-chart-sm/30">
                    <Calculator className="w-4 h-4" />
                    SM
                  </Badge>
                </TableCell>
                <TableCell className="text-right font-mono">
                  {fmt(row.consumReferinta)} lei
                </TableCell>
        ...
        {smData.length > 0 && (
          <TableFooter>
            <TableRow className="font-semibold">
              ...
              <TableCell className="text-right font-mono">{fmt(totals.consumReferinta)} lei</TableCell>
      ...
      <Dialog open={!!viewRow} onOpenChange={() => setViewRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Detalii Serviciul Mentenanță</DialogTitle>
          ...
          {viewRow && (
            <div className="space-y-3 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Consum referință (EE + A&C)</p>
                  <p className="font-mono font-semibold">{fmt(viewRow.consumReferinta)} lei</p>
                </div>
                <div>
      ...
      <Dialog open={!!editRow} onOpenChange={() => setEditRow(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Modificare Serviciul Mentenanță</DialogTitle>
          ...
          {editRow && (
            <div className="space-y-4 py-4">
              <div className="p-3 bg-muted rounded-lg">
                <p className="text-sm text-muted-foreground">Consum referință (EE + A&C)</p>
                <p className="font-mono font-semibold">{fmt(editRow.consumReferinta)} lei</p>
              </div>
