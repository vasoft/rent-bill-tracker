...
const ConsumptionNoteDocument = ({ client, period }: ConsumptionNoteDocumentProps) => {
  ...
  return (
    <div className="space-y-3 print:space-y-2 text-xs">
      {/* Header */}
      <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-md p-3 print:p-2">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-base font-bold text-foreground">NOTĂ DE CONSUM</h2>
            <p className="text-[11px] text-muted-foreground">Utilități și Servicii</p>
          </div>
          <div className="text-right">
            <p className="text-sm font-semibold capitalize">{formatPeriod(period)}</p>
            <p className="text-[11px] text-muted-foreground">Data emiterii: {new Date().toLocaleDateString('ro-RO')}</p>
          </div>
        </div>
      </div>

      {/* Client Info */}
      <div className="grid grid-cols-2 gap-3 p-2.5 bg-muted/30 rounded-md">
        <div className="space-y-1">
          ...
          <Badge variant={client.type === 'PJ' ? 'default' : 'secondary'} className="text-[10px] px-1.5 py-0">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-1.5 text-primary">
            ...
            <span className="font-semibold text-xs">Spații Închiriate</span>
          </div>
          <div className="flex flex-wrap gap-1">
            {client.clientSpaces.map(space => (
              <Badge key={space.id} variant="outline" className="text-[10px] px-1.5 py-0">
      ...
      {spaceDetails.map(space => (
        <div key={space.id} className="border border-border rounded-md overflow-hidden">
          <div className="bg-muted px-2.5 py-1.5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded bg-primary/10 flex items-center justify-center">
                <Gauge className="w-3 h-3 text-primary" />
              </div>
              <div>
                <p className="font-semibold text-xs">{space.name}</p>
                <p className="text-[10px] text-muted-foreground">{space.area} mp • {space.persons} {space.persons === 1 ? 'pers.' : 'pers.'}</p>
              </div>
            </div>
            <p className="font-bold text-sm">{formatNumber(space.spaceTotalValue)} lei</p>
          </div>

          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50 text-[10px] [&>th]:py-1 [&>th]:px-2">
                <TableHead>Utilitate / Serviciu</TableHead>
                <TableHead className="text-right">Consum</TableHead>
                <TableHead className="text-right">Val. Netă</TableHead>
            ...
            <TableBody>
              {space.utilities.map(u => (
                <TableRow key={u.utilityType} className="text-[11px] [&>td]:py-1 [&>td]:px-2">
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      <Badge variant="outline" className={`text-[10px] px-1 py-0 ${getUtilityColor(u.utilityType)}`}>
                        {u.utilityName}
                      </Badge>
                      <span className="text-[10px] text-muted-foreground">{u.fullName}</span>
                    </div>
                  </TableCell>
              ...
              <TableRow className="bg-muted/30 border-t-2 border-primary/20 [&>td]:py-1 [&>td]:px-2">
                <TableCell className="font-bold text-[11px]" colSpan={2}>TOTAL {space.name}</TableCell>
      ...
      <div className="border border-border rounded-md overflow-hidden">
        <div className="bg-primary/10 px-2.5 py-1.5">
          <p className="font-semibold text-xs text-primary">Sumar pe Utilități</p>
        </div>
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/30 text-[10px] [&>th]:py-1 [&>th]:px-2">
              <TableHead>Utilitate / Serviciu</TableHead>
          ...
          <TableBody>
            {summaryByUtility.map(u => (
              <TableRow key={u.id} className="text-[11px] [&>td]:py-1 [&>td]:px-2">
                <TableCell>
                  <div className="flex items-center gap-1.5">
                    <Badge variant="outline" className={`text-[10px] px-1 py-0 ${getUtilityColor(u.id)}`}>
                      {u.name}
                    </Badge>
            ...
            <TableRow className="bg-primary/5 border-t-2 border-primary/20 [&>td]:py-1 [&>td]:px-2">
              <TableCell colSpan={2} className="text-right font-bold text-xs">TOTAL DE PLATĂ:</TableCell>
              <TableCell className="text-right font-bold text-xs">{formatNumber(grandNet)}</TableCell>
              <TableCell className="text-right font-bold text-xs">{formatNumber(grandVat)}</TableCell>
              <TableCell className="text-right font-bold text-sm text-primary">{formatNumber(grandTotal)} lei</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
